...
class LedgerPresenter(object):
    ...
    def init_view(self):
        ...
        self.view.load_done_list(gbl.dataset.get_sent_invoices())
    ...
    def load_form(self):
        item = self.view.get_work_list_selection()
        if not item:
            return
    ...
    def save_form(self):
        ...
        updatable_flds = [
            ...
            'amount', 'short_code', 'grant_admin', 'grant_admin_email'
        ]
    ...
    def import_deposits(self):
        ...
        from views.deposits_dlg import DepositsDlg
        ...
        for idx in reversed(range(0, len(sheets))):
            ...
            for rownum in range(0, nrows):
                if sh.cell_value(rownum, 1).startswith('506'):
                    rex.append(Deposit(sh.row_values(rownum)))
        ...
        dlg = DepositsDlg(self.view, -1, rex)
        dlg.ShowModal()
        dlg.Destroy()
    ...
    def mark_done(self):
        ...
        gbl.dataset.send_invoice(form_vals['invoice_num'])

        self.view.send_invoice()

    def update_amount_balance(self, value):
        invoice = self.view.get_work_list_selection()
        invoice.amount = float(value)
        invoice.balance = float(value)